...
public class WaterSquare
{
    ...
    public WaterSquare (GameObject waterSquareObj, float size, float spacing)
    {
        ...
        // Because each square is 2 vertices, so we need to add one more
        width += 1;
    ...
    public void GenerateMesh()
    {
        ...
        for (int z = 0; z < width; z++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                // Get to the Coordinates of the Vertice
    ...
    // If we're updating the square from outside of a thread
    public void MoveSea(Vector3 oceanPos, float timeSinceStart)
    {
        ...
        for (int i = 0; i < vertices.Length;  i++)
        {
            ...
            // Don't need to go from Global to Local because the y pos is always set a 0
            vertices[i] = vertex;
         }
